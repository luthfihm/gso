<?	
	...
	$vw=$op->cs_query("SELECT * FROM bio ORDER BY no_daftar ASC");
	$i=1;
	...
	$excel->WriteToFile('biologi.xls');
	//$path="../upload/_jur/";
	header("Content-Disposition: attachment; filename=biologi.xls");
	header("Content-type: application/xls");
	$fp  = fopen("biologi.xls", 'r');
	$content = fread($fp, filesize("biologi.xls"));
	fclose($fp);
	echo $content;
	exit;
?>