<?	
	...
	$vw=$op->cs_query("SELECT * FROM fis ORDER BY no_daftar ASC");
	$i=1;
	...
	$excel->WriteToFile('fisika.xls');
	//$path="../upload/_jur/";
	header("Content-Disposition: attachment; filename=fisika.xls");
	header("Content-type: application/xls");
	$fp  = fopen("fisika.xls", 'r');
	$content = fread($fp, filesize("fisika.xls"));
	fclose($fp);
	echo $content;
	exit;
?>