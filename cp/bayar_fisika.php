<div id="judul_page"> <img src="../image/point.png" border="0" align="absmiddle" /> Data Pembayaran Bidang Fisika</div><br />
<style type="text/css">
...
<?
	$view=$op->cs_query("SELECT * FROM fis WHERE status=0 ORDER BY no ASC");
	$i=1;
...
if(isset($_POST['bayar'])){
	...
	$jum=count($id);
	for ($i=0; $i<$jum; $i++){
		$q=$op->cs_query("UPDATE fis SET status = 1 WHERE no_daftar ='$id[$i]'");
	}
	if($q){
