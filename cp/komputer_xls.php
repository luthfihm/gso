<?	
	...
	$vw=$op->cs_query("SELECT * FROM tik ORDER BY no_daftar ASC");
	$i=1;
	...
	$excel->WriteToFile('komputer.xls');
	//$path="../upload/_jur/";
	header("Content-Disposition: attachment; filename=komputer.xls");
	header("Content-type: application/xls");
	$fp  = fopen("komputer.xls", 'r');
	$content = fread($fp, filesize("komputer.xls"));
	fclose($fp);
	echo $content;
	exit;
?>